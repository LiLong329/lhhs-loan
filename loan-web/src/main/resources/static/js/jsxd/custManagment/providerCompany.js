$(function(){
     ...
     */
	 var $ur_a = $(".paginItem a");
		...
		function searcher(currentPageNo){
		   ...
		   window.location.href="/customerManager/providerCompany?"+obj;
	    }	
		/***检索部门***/
	...
	$(".companyModify").on("click",function(){
  	  if(!$("#myForm").validationEngine('validate')){
	     ...
	     $.post("/customerManager/providerCompanyModify",
	    		 loanProvider,function(data){
	          if(data.retCode=="00"){
	              alert(data.retMsg,function(){
	            	  window.location.href="/customerManager/providerCompany";
	              });
	          }else{
	              alert(data.retMsg);
	          }
