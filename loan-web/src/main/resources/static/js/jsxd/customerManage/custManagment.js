$(function(){
	...
	$("#saveBasicInfo").on("click",function(){
		...
		var borrower = $("#borrower").serializeJson(),
			bankList=[];
		$(".bankList").each(function(i,n){
			var temp={};
			temp.bankCardNo=$(n).find(".bankCardNo").val();
			temp.bankId=$(n).find(".bankName").val();
			temp.bankName=$(n).find(".bankName").val()?$(n).find(".bankName option:selected").html():"";
			temp.branchName=$(n).find(".branchName").val();
			bankList.push(temp);
		});
		borrower.bankList=bankList;
		// 更新页面数据
	...
	//基本信息-添加多个借款人银行卡信息
	$("#addBankInfo").on("click",function(){
		var html=$(this).closest("li").prop("outerHTML");
		$($("body .bankList").get($("body .bankList").length-1)).after(html);
		var temp=$($("body .bankList").get($("body .bankList").length-1));
		temp.find("input").val("");
		temp.find("select").val("");
		temp.find("em.uew-select-text").html("请选择");
		temp.find("#addBankInfo").prop("outerHTML",'<img class="del-zz" src="/img/loan_reduce.png">');
	});
	$("#tab1").on("click",".del-zz",function(){
	...
	for(var i=0;i<houseForm.length;i++){
			$(houseForm[i]).find("input[name='customerId']").val(customerId);
			$(houseForm[i]).find("input[name='custId']").val(custId);
			bankInfoList.push($(houseForm[i]).serializeJson());
	}
	if(bankInfoList.length >0){
		$.post("/customerManager/saveMortgageInfo",{
			"bankInfoList":JSON.stringify(bankInfoList),
			"customerId":customerId
		},function(data){
			if(data.retCode == "00"){
				alert(data.retMsg);
