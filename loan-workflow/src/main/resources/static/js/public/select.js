...
$("body").on("change",".provinceSelect",function(){
	...
	if(value&&cityOptions){
		strHtml+=option;
		strHtml+=cityOptions;
		$(this).closest("li").find(".citySelect").html("").html(strHtml).val("").prev().find("em")[0].innerHTML="请选择";
	}else{
		$(this).closest("li").find(".citySelect").html(option).val("").prev().find("em")[0].innerHTML="请选择";
	}
});
