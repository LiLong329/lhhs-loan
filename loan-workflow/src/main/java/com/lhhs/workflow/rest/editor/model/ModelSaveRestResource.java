 ...
 */
package com.lhhs.workflow.rest.editor.model;
...
public class ModelSaveRestResource implements ModelDataJsonConstants {
	...
	public void saveModel(@PathVariable String modelId,  HttpServletRequest values) {
		try {
			...
			modelJson.put(MODEL_NAME, values.getParameter("name"));
			modelJson.put(MODEL_DESCRIPTION, values.getParameter("description"));
			model.setMetaInfo(modelJson.toString());
			model.setName(values.getParameter("name"));

			repositoryService.saveModel(model);

			repositoryService.addModelEditorSource(model.getId(), values.getParameter("json_xml").getBytes("utf-8"));
